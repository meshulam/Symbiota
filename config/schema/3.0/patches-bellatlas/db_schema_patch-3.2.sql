        ...
        IF cha REGEXP '^[A-Za-z(),]' THEN
          IF latStart is not null and latEnd is not null and lngStart is not null THEN
            ...
            IF cha = ')' AND firstPt is not null and flipped not like CONCAT('%', firstPt) THEN
              -- mbaenrm modification: add last point to create closed polygons if needed
...
-- skip improperly formatted footprintWkt entries
UPDATE fmchecklists 
  SET footprintGeoJson = ST_ASGEOJSON(ST_GEOMFROMTEXT(swap_wkt_coords(footprintWkt))) 
  WHERE footprintGeoJson IS NULL AND footprintWkt IS NOT NULL AND footprintWkt LIKE 'POLYGON%';
